...
export async function requestMyHierarchy(context) {
  requestHelper(
    ...
    async () => {
      try {
        ['hierarchy_update', 'hierarchy_reset'].forEach(socketio.removeAllListeners)
      } catch (error) { }
      await api.get(`management/hierarchy`).then(({ data }) => {
        context.commit("set", data);
      });
      socketio.on('hierarchy_update', hierarchy => context.commit("update", hierarchy))
      socketio.on('hierarchy_reset', _ => api.get(`management/hierarchy`).then(({ data }) => {
        context.commit("set", data);
      }))
    },
    "Hierarchy",
  ...
  requestHelper(
    ...
    async () => {
      ...
      await api.get(`management/limit-request/${id}`).then(({ data }) =>
        context.commit('setManagement', { limitRequests: data, managementId: id })
      );
      socketio.on("limit_request_update", lr => context.commit("lrupdate", lr))
    },
    "Management",
    true
  )
