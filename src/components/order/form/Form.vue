<template>
  <div class="col column">
    <q-form @submit="onSubmit" @reset="resetForm" class="col column justify-between"
      ...
      <div class="col col-shrink q-gutter-y-sm column">
        <div class="col row q-gutter-x-sm items-center" v-if="_approvementMenuActive">
          ...
          <q-btn text-color="white" label="Отклонить" unelevated class="border-none bg-red col" @click="onDecline" no-caps
            dense />
        </div>
    ...
    " type="submit" no-caps dense :loading="_addLoading" :disable="_addLoading || _customerCreationCheck" />
          ...
          <q-btn v-if="_removeMenuActive && !currentUser?.role.includes('WATCHER')" text-color="white" label="Удалить"
            unelevated class="border-none bg-red col" @click="onRemoveOrder" no-caps dense />
        </div>
        <q-btn 
          v-if="_removeMenuActive && currentUser?.id == 1"
          text-color="white"
          label="Принудительно завершить"
          unelevated
          class="border-none bg-orange col"
          color="primary" @click="onComplete()"
          no-caps
          dense/>
      </div>
    </q-form>
...
export default {
  ...
  methods: {
    ...
    async onComplete() {
      ...
      console.log("Маршрут №", this.selected.id, " принудительно завершен")
    },
    async onApprove() {
      //
      if (this.checkAlerts()) return;
